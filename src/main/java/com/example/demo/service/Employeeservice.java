...
public class Employeeservice {
	...
	public List<EmployeeData> getEmployeeByName(String name) throws CustomException {
		List<EmployeeData> emp = Convertor.emptodata(repository.findByName(name));
	...
	public EmployeeData getEmployeeByMobile(String mobile) throws CustomException {
		Employee emp = repository.findByMobile(mobile)
				.orElseThrow(() -> new CustomException("No Employee with mobile number: " + mobile));
		...
		if (existingEmployee != null) {
			if (existingEmployee.getStatus().equals("Logged_in")) {
				try {
					...
					long t = Duration.between(existingEmployee.getLogintime(), existingEmployee.getLogouttime())
							.toSeconds();
					long t1 = Duration.between(existingEmployee.getLogouttime(), LocalDateTime.now()).toSeconds();
					if (t >= 0 && t1 >= 0) {
