...
public class Controller {
	...
	public String authenticateAndGetToken(@Valid @RequestBody AuthRequest authRequest) throws CustomException {
		Authentication authentication = authenticationManager.authenticate(
				new UsernamePasswordAuthenticationToken(authRequest.getUsermobile(), authRequest.getPassword()));
	...
	@PreAuthorize("hasAuthority('MANAGER')")
	public List<EmployeeData> findEmployeeByName(@RequestBody String name) throws CustomException {
	...
	@PreAuthorize("hasAuthority('EMPLOYEE')")
	public String updateEntryExitCode(HttpServletRequest request, @PathVariable String id,
			@RequestBody String entryexitcode) {
		String authHeader = request.getHeader("Authorization");
		String token = authHeader.substring(7);
		String usermobile = jwtService.extractUsermobile(token);
	...
	@PreAuthorize("hasAuthority('MANAGER')")
	public String updatelogout(@PathVariable String id, @RequestBody String time) throws CustomException {
		if (id.matches("\\d{1,6}") && Integer.parseInt(id) < 32768) {
			short id1 = Short.parseShort(id);
